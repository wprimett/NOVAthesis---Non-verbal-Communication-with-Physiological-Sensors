\section{BITalino R-IoT: Overview and Assessment}
...
% Where researchers may need to acquire data from many sources at once, we evaluate the use of the BITalino R-IoT.
% a low-cost sensory device designed for monitoring physiological activity in real-time.
...
\begin{table}[htbp]
    ...
    \begin{subtable}[c]{\textwidth}
        ...
        \label{table:latency_tab2}
     \end{subtable}
     \caption{Latency measurements for one device (5 metres).}
...
\begin{table}[htbp]
    ...
    \hfill
    % \hspace{2em}
    \caption{Latency measurements for four devices (1 metre).}
...
\begin{table}[htbp]
    ...
    \hfill
    % \hspace{2em}
    \caption{Latency measurements for four devices (5 metres).}
